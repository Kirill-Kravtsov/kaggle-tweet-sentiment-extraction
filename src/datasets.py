...
def process_data(tweet, selected_text, sentiment, tokenizer, max_len):
    ...
    try:
        idx_start = [m.start() for m in re.finditer(re.escape(selected_text), tweet)][0]
    except:
        print(tweet)
        print(selected_text)
    idx_end = idx_start + len_st - 1
    ...
    input_ids = [sentiment_id[sentiment]] + [2] + [2] + input_ids_orig + [2]
    token_type_ids = [0] * (3 + len(input_ids_orig) + 1)
